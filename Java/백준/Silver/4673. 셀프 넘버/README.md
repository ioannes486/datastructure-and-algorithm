...
### 출력 
 ...
 브루트 포스를 이용한 알고리즘은 제어문만 잘 쓰면 된다는 교훈을 얻었다. 처음에는 ArrayList를이용해보려고 했지만 이내 다른 해답을 얻었다. 브루트포스 알고리즘을 연습함으로써 모든 문제를 완전탐색으로 구현할 수 있는 기초를 쌓기 위해 이 문제를 선택했다. 풀고나니 브루트포스에 대한 감을 어느정도 얻을 수 있었던 값진 문제였다. 
